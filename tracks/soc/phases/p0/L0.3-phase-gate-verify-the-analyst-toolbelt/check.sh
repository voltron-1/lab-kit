#!/usr/bin/env bash
set -euo pipefail
: "${LAB_WORKSPACE:?run this via: lab check soc L0.3}"
: "${LAB_CHECKLIB:?run this via: lab check soc L0.3}"
# shellcheck source=/dev/null
source "$LAB_CHECKLIB"

# --- BEGIN GENERATED KEY (genevidence: s0-tier-cases) ---
KEY_Q1_B64="ZQ=="
KEY_Q2_B64="aA=="
KEY_Q3_B64="aA=="
KEY_Q4_B64="ZQ=="
KEY_Q5_B64="aA=="
# --- END GENERATED KEY ---

K1="$(echo "$KEY_Q1_B64" | base64 -d)"
K2="$(echo "$KEY_Q2_B64" | base64 -d)"
K3="$(echo "$KEY_Q3_B64" | base64 -d)"
K4="$(echo "$KEY_Q4_B64" | base64 -d)"
K5="$(echo "$KEY_Q5_B64" | base64 -d)"

assert_cmd_ok "jq responds" "install jq — see L0.1" -- jq --version
assert_cmd_ok "tshark responds" "install tshark — see L0.1" -- tshark --version
assert_cmd_ok "dig responds" "install dnsutils — see L0.1" -- dig -v
assert_cmd_ok "whois responds" "install whois — see L0.1" -- whois --version
assert_cmd_ok "rg responds" "install ripgrep — see L0.1" -- rg --version

assert_file_exists "answers.txt"
assert_file_exists "cases.md"
assert_file_exists "sop-excerpt.md"

assert_file_contains "answers.txt" '^q1='
assert_file_contains "answers.txt" '^q2='
assert_file_contains "answers.txt" '^q3='
assert_file_contains "answers.txt" '^q4='
assert_file_contains "answers.txt" '^q5='

tr '[:upper:]' '[:lower:]' < answers.txt | sed 's@#.*@@' | tr -d '\r' > .answers.norm

assert_file_contains ".answers.norm" "^q1=$K1$"
assert_file_contains ".answers.norm" "^q2=$K2$"
assert_file_contains ".answers.norm" "^q3=$K3$"
assert_file_contains ".answers.norm" "^q4=$K4$"
assert_file_contains ".answers.norm" "^q5=$K5$"

ck_summary
