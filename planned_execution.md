...
**Four real, previously-undetected bugs found and fixed** (this is the project's now-established pattern: a new invariant finds something real):
1. **Three scenario windows too narrow.** `s1-telemetry`, `s2-http-tls`, and `s2-gate-session`'s `window:` all started AFTER benign filler rows the generator itself emits (e.g. `s2-http-tls`'s window started 15:45Z while its own `updates.example`/`saas.mail.example` noise rows are timestamped 09:00-09:15Z the same day) — confirmed by decoding the actual emitted zeek/JSON timestamps against each scenario's own declared window, not just eyeballing the yaml. Fixed by widening each `window.start` to the true earliest row.
2. **`build_pcap_dns`'s answer-IP formula was silently wrong.** L0.1's `fixtures.pcap` answered `fs01.coppermine.example`/`dc01.coppermine.example` DNS queries with `10.20.10.7`/`10.20.10.8` — neither matching either host's real universe.yaml IP (FS01=.8, DC01=.5) — because the formula derived the answer from the query's list *index* (`10.20.10.5 + idx`), which only coincidentally matched the first entry. Confirmed via `tshark -T fields -e dns.qry.name -e dns.a` on the actual emitted pcap. Fixed by making `dns_pcap_queries` carry an explicit real answer IP per qname instead of guessing one.
3. **Inconsistently-defanged "raw" evidence.** `cases.md` (L0.3) and `incident-brief.md` (L1.6) are genevidence's "markdown-cards"/narrative-evidence emitters — same "must look real, never defanged" rule as a zeek log — but CASE 3 and three of `incident-brief.md`'s seven action lines had a defanged domain/scheme sitting next to a raw IP in the same sentence (e.g. `c2.stonewick[.]example (203.0.113.66:443)`), plus one `[.]`-defanged IP in L0.1's `triage-notes.txt`. Fixed by un-defanging all of them back to raw, consistent with every other line in the same files.
4. **L1.3 was the one lab in the whole 52-lab track whose graded IP-address answer was never defanged.** `q4`'s answer key was the raw `203.0.113.66`, and `lab.md`'s own answer spec quoted the same raw form — while every other IP/domain answer across the track (L0.2, L2.1, L2.5, L2.6, L2.7, L2.4, L4.2) already defangs. Confirmed by comparing every genevidence check.sh's decoded `KEY_*_B64` values against `universe.yaml`'s external IOCs — this was the only hit. Fixed the scenario yaml's answer key, `lab.md`'s three answer-spec mentions, and `answers.template.txt`'s hint comment, then re-ran `genevidence.py` to resync `check.sh`; also had to update `tests/acceptance.sh`'s L1.3 fabricated-pass fixture to submit the now-defanged form (missing this first caused a frontier-lock cascade that failed every soc lab after L1.3 — caught by re-running the full suite, not assumed clean).

Also widened `universe.yaml`'s `externals:` with two previously-unregistered-but-heavily-reused benign hosts (`saas.mail.example`/192.0.2.60, `updates.example`/192.0.2.61 — used as filler traffic across L1.1/L2.1/L2.2/L2.3/L2.5/L2.6/L2.7) and gave `check_ips_hosts_resolve` a `*.<org-domain>` wildcard so `www.coppermine.example` resolves without needing individual registration — both are the "obviously non-attributable/benign, not a real leak" case the task asked me to use judgment on, not new attacker infrastructure.

Suite: `./tools/lint-labs.sh` clean, `bash tests/acceptance.sh` still **1069/1069**. Landed as commits directly on the session's working branch per this session's harness instruction (no new branch, no PR, nothing pushed).

2026-08-29 — ps p0-p3 CLOSE-OUT: closed the "ps p0-p3 have zero `tests/acceptance.sh` coverage" gap flagged above and first noted in the 2026-07-28 tracker-reconciliation entry below. Added four new sections (p0's 3 labs, p1's 8, p2's 7, p3's 8 — 26 total) inserted directly before the existing ps track P4 section, mirroring the P4-P7 pattern already in the file and the soc P0+P1 section's from-scratch approach for phase openers with no earlier-track-lab to recall. Every lab gets a fail-before-artifacts case and a fabricated pass + quiz; five labs also get a dedicated negative/regression case for the one real grading-integrity control in that phase: L0.3's N-of-4 Execution-Policy-bypass-technique threshold, L1.2's live-reexecution proof (a `type.ps1` tampered to claim the wrong .NET type must fail — proves check.sh actually reruns the shipped probe through real pwsh rather than trusting its mere presence), L2.4's advanced-function reflection control (stripping every `[CmdletBinding()]`/`[Parameter(...)]` decorator from `tool.ps1` must flip `probe.ps1`'s real `(Get-Command ...).Parameters.ContainsKey('Verbose')` reflection result to `False` — this lab's own lesson is that such decorators are a "legitimacy costume", not a safety guarantee, so the test proves the grader tracks the real reflection outcome and can't be gamed by leaving the word "Verbose" in a learner file), and L2.7/L3.8's N-of-6/N-of-8 comprehension-question thresholds. Every lab shipping a live-executed reference `.ps1` probe (L1.2, all of L1.4, L2.1/L2.2/L2.3/L2.5/L2.6, L3.1/L3.3/L3.6) was verified against the real pwsh 7.4.6 installed in this environment, not fabricated from memory or copied from lab.md's prose.

**A real, 100%-reproducible bug found and fixed, not just theoretical**: ps L1.4's shipped `files/calc.ps1` (`Get-Process -Name pwsh | Select-Object -First 1 Name, @{Name='MB';Expression={...}}`) relies on PowerShell's *implicit* default table formatting. `$Host.UI.RawUI.WindowSize` reads `-1x-1` in the exact environment `lab check` always runs check.sh in (lib/workspace.sh's `env -i` fence, stdin redirected from `/dev/null`, no controlling tty — a fixed, deliberate property of *every* check.sh invocation by design, not a quirk of this session's sandbox) — and when that read is invalid, `Format-Table` silently emits nothing at all, not even a header row. `expand.ps1`/`itype.ps1`/`seltype.ps1` (bare single-value output, no implicit table) were unaffected and always passed; only `calc.ps1`'s multi-property `Select-Object` output hit this. Net effect: **no real learner could ever have passed L1.4** by doing exactly what its own `lab.md` instructs — the reference script check.sh grades against was silently producing empty output every single time, confirmed 100% reproducible (6/6, then reconfirmed 8/8) once driven directly through the real `lab` CLI with the same prior-lab call history a learner's session would have. First surfaced as an apparent one-off flake on this session's very first full acceptance run; proven genuinely deterministic, not random, once isolated. Fixed by piping through `Format-Table -AutoSize | Out-String -Width 200` to force an explicit width and bypass the broken auto-detection — re-verified clean 6/6 on a fresh repo copy, then clean across a full `tests/acceptance.sh` run.

**A second bug, in this session's own new code, caught before it could go stale-green**: the ps track's final `status shows all 54 ps P0-P7 labs passed (54/54)` assertion was a wrong-reason-pass trap of the bash track's own making — bash also totals 54 labs, and its status line has read `(54/54)` since bash's own P0-P7 close-out early in the same run, so a bare `(54/54)` substring check against the whole multi-track `lab status` output would trivially pass off *bash's* frozen line even if ps itself had regressed entirely. This is exactly what happened on this session's first full run: the L1.4 bug above cascaded a frontier-lock through the rest of the ps track (every later lab's `lab start` refused, workspace never provisioned, `lab check` returned "not started" instead of a graded result) — yet this one assertion still reported PASS, off bash's unrelated line. Fixed by grepping the `^ps ·` status line specifically before matching; confirmed the fix has teeth by re-running it against the still-broken (pre-`calc.ps1`-fix) state and watching it correctly report the real failure instead of a false pass.

Suite: 931 → **1069 passing, 0 failing** — confirmed on a full from-scratch `tests/acceptance.sh` run after both fixes above, not just by inspection (two earlier full runs, both before the `calc.ps1` fix, read 836/233 and 835/234 — the flake was that consistent). `--force` dropped from all four ps phase-opener start calls it had accreted (L4.1, and — turns out unnecessarily, confirmed empirically — L5.1/L6.1/L7.1 too): `catalog_frontier` treats a *skipped* lab as unlocking-complete exactly the same as a *passed* one, so the single force-skip at the true first `ps start` call in a fresh COPY is sufficient to unlock the whole track linearly once p0-p3 pass for real ahead of it — the three downstream `--force` flags were leftover caution from before p0-p3 had coverage, never actually load-bearing. Landed as commits directly on the session's working branch per this session's harness instruction (no new branch opened, no PR, nothing pushed).
...
2026-08-08 — soc p3 BUILD + CLOSE-OUT (all 7 labs, L3.1–L3.7 complete). Built straight from `docs/plans/soc-p3-plan.md`.
- **Scaffolded and verified all 7 labs**: L3.1 Windows Security Events (4624/4625, logon types, 4688, 4720/4732), L3.2 Sysmon Process Trees (`ProcessGuid`/`ParentProcessGuid`), L3.3 Anomalous Process Ancestry (Office/browser spawning shells), L3.4 Persistence Spots (Run keys, scheduled tasks, services, cron), L3.5 Linux Auth & Audit (`auth.log`, SSH brute force, sudo abuse, UID 0), L3.6 LOLBins (encoded PowerShell, certutil urlcache download, curl | bash), L3.7 Phase Gate Endpoint Compromise Chain.
- **Linting & Acceptance**: `./tools/lint-labs.sh` and `./tools/shellcheck-all.sh` clean. Added SOC P3 acceptance section to `tests/acceptance.sh` (7 labs, fabricated pass + negative cases, denominator updated to 25). Full test suite green: 805/805 passing.
- **Tagged `soc-p3`**.
- **Scaffolded all 8 labs**: L5.1 threads & data races (CWE-362, `error[E0373]`), L5.2 Send/Sync (`error[E0277]`), L5.3 Arc/Mutex, L5.4 mpsc channels, L5.5 async/await mental model (`tokio` 1.53), L5.6 tokio `JoinSet` main loop, L5.7 async DoS audit (CWE-400, timeouts/semaphores), L5.8 phase gate (tracing concurrent port scanner data flow).
- **Verified with real toolchain**: std labs verified with bare `rustc`; tokio labs (L5.5, L5.6) compiled and verified with `cargo run` using `tokio` 1.53. All outputs, compile errors, and deterministic calculations confirmed.
- **Linting & Acceptance**: `tools/lint-labs.sh` and `tools/shellcheck-all.sh` clean. Added Rust P5 section to `tests/acceptance.sh` (8 labs, fabricated pass + negative cases, denominator updated to 50). Full suite green: 685/685 passing.

Earlier, 2026-07-31 — ps p7 BUILD (L7.7) + CLOSE-OUT, completing the ps track (this session, continuing directly from the ps p6 close-out session below). Opened by re-reading this file's own NEXT UP, which turned out to be stale: it said the p7 plan had "not been verified against real pwsh or approved for build," but `git log` showed plan-verify (PR #382) and L7.1–L7.6 (PRs #383–388) already merged to `main` — only L7.7 (the capstone gate, and the track's final lab) was unbuilt. Tracker drift, not build state; corrected here rather than re-verifying an already-verified plan.
- **Built L7.7-capstone-gate** (PR #389) on the branch a prior session had already checked out for it. Grades the L7.6 `hardened.ps1` (structural bar + no bare code-execution alias), a real `Invoke-ScriptAnalyzer` pass (`pssa-clean.txt`), and a self-audit against the L7.3 checklist (`answers.md`). `security-auditor` + `code-reviewer` ran in parallel and both came back with real, non-theoretical grading-integrity gaps: a missing-file false "clean" pass message; a 0-byte "PSSA clean" bar that couldn't distinguish "found nothing" from "PSSA never ran" (module missing, bad path, and mid-run errors all vanish through a bare `>` redirect — the plan's own §7 records PSSA wasn't preinstalled at build time, so this was a real gap on a fresh machine, not a theoretical one) — fixed by adding `pssa-version.txt` as a proof-of-run artifact, mirroring L7.5's existing pattern; `answers.md`'s self-audit made the one security-critical item (naming the banned alias) optional under a 4-of-5 threshold — promoted to mandatory, matching the precedent L6.5 set for its own load-bearing item; the alias ban itself was a plain-text scan defeatable by `[ScriptBlock]::Create(...)` or `&$cmd`/`. $cmd` — the exact obfuscation techniques this track teaches at L5.2 — closed by reusing the two bans `tools/lint-labs.sh` already enforces on shipped content; "logging on" was satisfiable by the comment `# TODO: add logging` (a gap the plan's §7 had already flagged and left unresolved); and `[CmdletBinding()]` false-failed valid argument variants like `[CmdletBinding(SupportsShouldProcess)]`. All six re-verified against real pwsh 7.6.4 + PSScriptAnalyzer 1.25.0 and the real `lab` CLI: fail path, pass path + quiz, and six targeted negative cases (bare-IEX bypass, `ScriptBlock::Create` bypass, call-operator bypass, comment-only-logging bypass, missing `pssa-version.txt`, `answers.md` skipping the mandatory alias mention).
- The last three fixes (logging, `[CmdletBinding()]`, obfuscation hardening) were **backported to the already-merged `L7.6-capstone-direct/check.sh`** in the same PR — it asserts the identical `hardened.ps1` security properties and had the identical gaps. Re-verified L7.6 end to end after the backport (fail path, pass path + quiz) to confirm the already-shipped lab still works. PR #389 confirmed `MERGEABLE`/`CLEAN` before merge, per the standing rule that merging needs explicit go-ahead.
- **CLOSE-OUT**, own branch+PR: found and fixed a **pre-existing red suite** before adding new coverage — three stale ps-catalog-denominator assertions (`(9/53)`, `(16/53)`, `(21/53)`) were still asserting p6's total even though all 7 p7 lab directories (L7.1–L7.7) already existed on disk across PRs #383–389; bumped all three to `(_/54)`, the same "every new lab dir shifts the denominator" class of regression called out for p5 and p4 before it, caught here by actually running the suite rather than by inspection. Then added a `tests/acceptance.sh` ps track P7 section (7 labs, fabricated pass + negative case each, mirroring the P4–P6 pattern) — no lab in this phase runs PSSA or pwsh live in `check.sh` (pure static grading throughout), so the section needed no pwsh preflight. The one dedicated negative case regression-tests the capstone's security-critical control: an ALLCAPS `IEX $payload` must still fail L7.7's gate, proving the case-insensitivity fix (and this session's hardening on top of it) holds under repeat runs, not just at build time. Suite: 526 → **529 passing** before the new section, adding coverage for all 7 p7 labs on top. Tagged `ps-p7` on the close-out merge commit — **the ps track (p0–p7, 54 labs) is now fully built, reviewed, tested, and tagged.**

Earlier, 2026-07-31 — ps p6 CLOSE-OUT (the p6 build itself — plan-verify PR #374, L6.1–L6.4 PR #375, L6.5 phase gate PR #376 — was already merged to `main` from a prior session, but `planned_execution.md` had not caught up and `tests/acceptance.sh` had no P6 coverage yet). Local `main` was a fast-forward behind `origin/main` by the L6.5 merge commit; synced before starting. Added a `tests/acceptance.sh` ps track P6 section (5 labs, fabricated pass + negative case each, mirroring the P4/P5 pattern) via its own branch+PR. Unlike P4/P5, no lab in this phase runs pwsh at all — every check.sh is pure static grading of the learner's tour writeup, so the section needed no pwsh preflight. The one new structural control, L6.5's phase-gate byte-anchoring of its two shipped tour files, got a dedicated negative case (tamper + restore), the P6 analogue of P5's tampered-probe coverage. Catalog denominator stays `(_/47)` through this close-out since p6's 5 lab directories were already on disk (and already counted) before P5's own close-out landed. Tagged `ps-p6` on the merge commit.

Earlier, 2026-07-29 — ps p5 BUILD-OUT + CLOSE-OUT, finishing the phase in one session (continues the L5.4 session below).
...
- [x] rust p4 — Security-Critical Rust (10 labs) — commit `b233fc2` (L4.1–L4.10; landed direct-to-main, bundled with soc L2.2, not yet reviewed or tagged — see NEXT UP)
- [x] rust p5 — Concurrency & Async (8 labs) — plan: `docs/plans/rust-p5-plan.md`; L5.1–L5.8 built, tested with real toolchain + tokio 1.53, acceptance.sh P5 section added (685/685 tests green)
- [x] rust p6 — Reading Real Security Tools (6 labs) — plan: `docs/plans/rust-p6-plan.md`; L6.1–L6.6 built, tested, and acceptance suite verified (712/712 tests green)
- [x] rust p7 — Directing & Auditing AI Rust (7 labs) — tag `rust-p7`; plan: `docs/plans/rust-p7-plan.md`; L7.1–L7.7 built, tested, capstone ECS parser verified, acceptance suite green (739/739 tests)

**rust track complete: p0–p7, 63 labs, all built and verified.**
...
- [x] ps p6 — Reading Real Security Tools (5 labs) — tag `ps-p6`; plan: `docs/plans/ps-p6-plan.md` (PR #263; verified vs real pwsh + approved for build, PR #374); L6.1 tour PowerView, L6.2 tour IR collector, L6.3 tour threat hunt, L6.4 tour Sigma rule (PR #375, all four non-gate tours), L6.5 phase gate cold tour (PR #376); close-out: `tests/acceptance.sh` P6 section (PR #381)
- [x] ps p7 — Directing & Auditing AI PowerShell (7 labs) — tag `ps-p7`; plan: `docs/plans/ps-p7-plan.md` (PR #267; verified vs real pwsh + approved for build, PR #382); L7.1 AI failure patterns (PR #383), L7.2 safe-PS spec (PR #384), L7.3 review checklist (PR #385), L7.4 review reps (PR #386), L7.5 CI guardrails (PR #387), L7.6 capstone direct (PR #388), L7.7 capstone gate (PR #389, also backports L7.6 hardening); close-out: `tests/acceptance.sh` P7 section (PR #390)

**ps track complete: p0–p7, 54 labs, all tagged.**
