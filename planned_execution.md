...
## NEXT UP
Track: **ps p6 — Reading Real Security Tools** is **fully closed out** (5/5 labs, `tests/acceptance.sh` P6 section, tag `ps-p6`). Next unstarted item: **ps p7 — Directing & Auditing AI PowerShell** (7 labs, the ps track's final phase); its build plan `docs/plans/ps-p7-plan.md` (PR #267) is written but has **not** been verified against real pwsh or approved for build — do that first, the way PR #374 did for p6 and PR #363 did for p5. The plan's own `[VERIFY-AT-BUILD]` flags (§7 runtime facts, exact PSSA rule names, "clean run" output shape) are exactly what that verify pass needs to resolve.

L7.1's `recall.json` (5 Qs from Phases 5+6) is specified in the p7 plan itself (§189, `[VERIFY-AT-BUILD]`, reconcile with the ps-p6 L6.5 build) rather than drafted ahead in a prior phase's handoff — reconcile it against what L6.1–L6.5 actually shipped before use, the same caution the plan already flags.

Build-step reminder, still live for p7: **every new lab directory shifts `tests/acceptance.sh`'s ps catalog denominator**, because `lab status` counts lab directories present on disk, not committed progress. During p5, L5.1–L5.3 each merged without bumping it and left the suite red across three PRs. Bump it in the *same* PR as each lab — p7 starts from `(21/47)` and goes to `(21/48)`, `(21/49)`, and so on up to `(21/54)` once all 7 lab directories exist. The same file also asserts the count of unstarted `[ ]` track-phase lines in this document, so change a phase marker and update that assertion too.
